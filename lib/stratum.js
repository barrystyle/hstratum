...
//! add sqlite3 for share/round keeping
var sqlite3 = require('sqlite3').verbose();
let db = new sqlite3.Database('./hnd-stratum', (err) => {
  if (err) {
    console.error(err.message);
  }
  console.log('Connected to the handshake sharedata db.');
});
...
class Stratum extends EventEmitter {
  ...
  async handleSubmit(conn, msg) {
    ...
    let PoolDiff = diffTarget;
    const difficulty = common.getDifficulty(share.powHash());
    if(job.target.compare(diffTarget) > 0){
      diffTarget = job.target;
    }

    const submitShare = share.powHash();
    if(submitShare.compare(PoolDiff) > 0 &&
       submitShare.compare(job.target) > 0)
    {
      this.logger.debug(
    ...
    //! basic format sqlite3 table

    //    CREATE TABLE hndshares (
    //        id          INTEGER      PRIMARY KEY AUTOINCREMENT,
    //        username    VARCHAR (64) NOT NULL,
    //        unixtime    TIME         NOT NULL,
    //        sharediff   TEXT (64)    NOT NULL,
    //        sharehash   TEXT (64)    NOT NULL,
    //        sharetarget TEXT (64)    NOT NULL
    //    );

    // if we made it here, log the share unconditionally
    db.run(`INSERT INTO hndshares (username, unixtime, sharediff, sharehash, sharetarget) VALUES (?, ?, ?, ?, ?)`, 
           [subm.username, subm.ts, difficulty, share.powHash().toString('hex'), job.target.toString('hex')]);
